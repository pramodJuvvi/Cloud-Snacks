...
export function CartScreen() {
  ...
  const placeOrder = async () => {
    ...
    const customerEmail =
      checkoutDetails.customerEmail.trim() || `${customerPhone || "customer"}@cloudsnacks.local`;
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      >
        <AppHeader
          ...
          title="Your snack bag"
          subtitle={
            totalItems > 0
              ? `${totalItems} snack${totalItems === 1 ? "" : "s"} selected. Add address, offers, and payment in one clean flow.`
              : "Add favorites from Today's Picks and finish checkout here."
          }
        >
        ...
        <SectionPanel
          ...
          title="Cart Items"
          subtitle="Adjust quantities before placing the order."
        >
        {items.length === 0 ? (
          <View style={styles.emptyState}>
            <Ionicons name="bag-handle-outline" size={42} color={theme.colors.muted} />
            <Text style={styles.emptyTitle}>No snacks yet</Text>
            <Text style={styles.emptyText}>Add something from Today&apos;s Picks and it will appear here.</Text>
          </View>
        ) : (
