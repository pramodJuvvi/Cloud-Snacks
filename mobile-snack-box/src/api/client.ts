import { snacks as subscriptionBoxes } from "../data/snacks";
...
const localSubscriptionOrders: OrderHistoryItem[] = [];
...
async function request(url: string, options: RequestInit = {}) {
  ...
  try {
    ...
    throw new ApiError("We could not connect to Cloud Snack Box. Please check your internet and try again.");
  } finally {
...
function friendlyStatusMessage(status: number) {
  ...
  if (status >= 500) {
    return "Cloud Snack Box is temporarily unavailable. Please try again shortly.";
  }
...
export async function getBackendStatus(): Promise<string> {
  ...
  return data.message ?? "Cloud Snack Box backend reachable";
}
...
export async function getBackendSnacks() {
...
export async function createOrder(
  ...
  const localItems = items
    .map((item, index) => {
      const box = subscriptionBoxes.find((subscriptionBox) => Number(subscriptionBox.id) === item.snackId);
      if (!box) {
        return null;
      }
  ...
  if (localItems.length === items.length && localItems.length > 0) {
    const subtotal = localItems.reduce((total, item) => total + item.unitPrice * item.quantity, 0);
    const couponCode = details.couponCode.trim().toUpperCase();
    const discount = couponCode === "FIRST50" ? Math.min(50, subtotal) : 0;
    const order: OrderHistoryItem = {
      ...
      couponCode,
      discount,
      deliveryPartner: "Subscription concierge",
      feedbackRating: null,
      feedbackNote: null,
      issueReport: null,
      items: localItems,
    };
    localSubscriptionOrders.unshift(order);
    return order;
  }
...
export async function getMyOrders(accessToken: string): Promise<OrderHistoryItem[]> {
  if (localSubscriptionOrders.length > 0) {
    return localSubscriptionOrders;
  }
